    ...
    // Or adjust the layout logic if 'Add' is a real screen
    const displayItems = navItems; //.filter(item => !item.isCenter); // Example: filter if 'Add' is not a screen

    // Re-insert the center button in the middle if it was filtered
    // const centerButton = navItems.find(item => item.isCenter);
    // if (centerButton) {
    //   displayItems.splice(Math.floor(displayItems.length / 2), 0, centerButton);
    // }
    ...
    bottomNav: {
        ...
        height: Platform.OS === 'ios' ? 60 + SPACING.LARGE : 60, // Adjust height (add safe area padding for iOS)
        paddingBottom: Platform.OS === 'ios' ? SPACING.LARGE : 0, // Safe area padding for iOS bottom notch
        // Appearance
        borderTopWidth: 1,
        borderTopColor: COLORS.lightGrey, // Subtle top border
        shadowColor: COLORS.black, // Optional shadow for depth
    ...
    navItem: {
        ...
        paddingVertical: SPACING.SMALL, // Vertical padding within the item
        height: '100%', // Take full height of the bar
    },
    navText: {
        fontSize: FONT_SIZES.SMALL,
        fontFamily: FONT_FAMILY.MEDIUM,
        marginTop: SPACING.XSMALL, // Space between icon and text
    },
    // Styles for the distinct center button
    navButton: {
        flex: 1, // Also take equal width
        alignItems: 'center',
        justifyContent: 'center', // Center the gradient button
        // Adjust position if needed (e.g., slightly raised)
        // marginBottom: 15,
        height: '100%',
    },
    navButtonGradient: {
        width: 54, // Size of the circular button
        height: 54,
        borderRadius: 27, // Make it circular
        alignItems: 'center',
        justifyContent: 'center',
        // Move button slightly up from the baseline
    ...
    // Optional style for text below center button
    // navCenterText: {
    //   position: 'absolute',
    //   bottom: Platform.OS === 'ios' ? SPACING.LARGE + SPACING.XSMALL : SPACING.XSMALL, // Position below the main bar line
    //   color: COLORS.grey, // Or COLORS.primary if it should be highlighted
    // },
    });
